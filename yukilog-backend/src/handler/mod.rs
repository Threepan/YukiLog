...
pub fn public_routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/themes/:slug", get(public::themes::get_theme))
        .route("/api/themes/:slug/view", post(public::themes::increment_theme_view))
        // Tags (3个)
        .route("/api/tags", get(public::tags::list_tags))
        .route("/api/tags/:slug", get(public::tags::get_tag))
        .route("/api/tags/:slug/view", post(public::tags::increment_tag_view))
        // Posts (3个)
        .route("/api/posts", get(public::posts::list_posts))
        .route("/api/posts/:slug", get(public::posts::get_post))
        .route("/api/posts/:slug/view", post(public::posts::increment_post_view))
        // Comments (3个)
        .route("/api/posts/:slug/comments", get(public::comments::get_post_comments))
        .route("/api/posts/:slug/comments", post(public::comments::create_comment))
        .route("/api/posts/:slug/comments/:id", get(public::comments::get_comment_replies))
        // Links (2个)
        .route("/api/links", get(public::links::list_links))
        .route("/api/links/submit", post(public::links::submit_link))
}
